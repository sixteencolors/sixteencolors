...
-- Created on Mon Jan 26 20:38:45 2015
-- 
...
CREATE TABLE artist_tag (
  ...
  ctime timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
  mtime timestamp NULL,
  INDEX artist_tag_idx_artist_id (artist_id),
  PRIMARY KEY (artist_id, tag),
  CONSTRAINT artist_tag_fk_artist_id FOREIGN KEY (artist_id) REFERENCES artist (id) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB;
...
CREATE TABLE file (
  ...
  INDEX file_idx_root_id (root_id),
  PRIMARY KEY (id),
...
CREATE TABLE group_tag (
  ...
  ctime timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
  mtime timestamp NULL,
  INDEX group_tag_idx_group_id (group_id),
  PRIMARY KEY (group_id, tag),
  CONSTRAINT group_tag_fk_group_id FOREIGN KEY (group_id) REFERENCES grp (id) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB;
...
CREATE TABLE pack_tag (
  ...
  ctime timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
  mtime timestamp NULL,
  INDEX pack_tag_idx_pack_id (pack_id),
  PRIMARY KEY (pack_id, tag),
  CONSTRAINT pack_tag_fk_pack_id FOREIGN KEY (pack_id) REFERENCES pack (id) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB;
...
CREATE TABLE file_tag (
  ...
  ctime timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
  mtime timestamp NULL,
  INDEX file_tag_idx_file_id (file_id),
  PRIMARY KEY (file_id, tag),
  CONSTRAINT file_tag_fk_file_id FOREIGN KEY (file_id) REFERENCES file (id) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB;
